...
"""
A test program that runs queries sequentially or in parallel.
The queries will run with the database we have on the IN2P3 cluster
for the Summer 2015 test.
...
for i in range(0, 10):
    ...
    # single object
    deep_source_id = random.randint(2251799813685248, 4503595332407303)
    sql = "SELECT ra, decl, raVar, declVar, radeclCov, "
    "u_psfFlux, u_psfFluxSigma, u_apFlux FROM Object "
    "WHERE deepSourceId = {}".format(deep_source_id)
    QUERY_POOLS["LV"].append(sql)
    ...
    sql = "SELECT ra, decl, raVar, declVar, radeclCov, "
    "u_psfFlux, u_psfFluxSigma, u_apFlux FROM Object "
    "WHERE qserv_areaspec_box({}, {}, {}, {})".format(
        raMin,
        declMin,
        raMin+raDist,
        declMin+declDist)
    ...
    sql = "SELECT ra, decl, raVar, declVar, radeclCov, "
    "u_psfFlux, u_psfFluxSigma, u_apFlux FROM Object "
    "WHERE qserv_areaspec_box({}, {}, {}, {}) "
    "and o.deepSourceId = s.objectId".format(
        raMin,
        declMin,
        raMin+raDist,
        declMin+declDist)
